...
router.use("/", (req, res, next) => {
  // const apiKey = req.queryStringParameters.apiKey;
  console.log("inside download server");
  const sheets = ["Confirmed", "Recovered", "Deaths"];

  const sheetQueries = sheets.map(sheetName => {
    return fetch(
      `https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-${sheetName}.csv`
    ).then(d => d.text());
  });
